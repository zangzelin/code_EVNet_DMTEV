...
class TemperatureAdjusterHook(tf.estimator.SessionRunHook):
...
def run_experiment(name, train, val, test, K, hidden_units, num_epochs, batch_size, learning_rate, dropout, start_temp = 10.0, min_temp = 0.01, tryout_limit = 1):
    import random
    import os 
    ...
    for i in range(tryout_limit):
        model_dir = ('./wandb/concrete_model_dir_' + name + '_' + str(num_epochs) + '_' + str(datetime.now())).replace(' ', '_').replace(':', '.')
        ...
        regressor = tf.estimator.Estimator(
            ...
            config = tf.estimator.RunConfig(
                save_checkpoints_steps = epochs_per_evaluation * steps_per_epoch, 
        ...
        test_mse = mean_squared_error(
            [pred for prediction in predictions for pred in prediction['predictions']], test[1][perm])
