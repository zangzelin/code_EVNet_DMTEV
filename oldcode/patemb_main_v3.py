...
import Loss.dmt_loss_aug3 as dmt_loss_aug
...
class LitPatNN(LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        # self.Loss1 = dmt_loss_aug1.MyLoss(
        #     v_input=100,
        #     metric=self.hparams.metric,
        #     augNearRate=self.hparams.augNearRate,
        # )
        self.tpnn = nn.Linear(100, 80)
        
        # self.c_nn = nn.Linear(100, 2, bias=False)
        ...
        # decoder = []
        # decoder.append(nn.Linear(80, 1000))
        # decoder.append(nn.LeakyReLU(0.1))
        # decoder.append(nn.Linear(1000, self.data_train.data.shape[1]))
    ...
    def updata_m(self, init=False):
        ...
        if init:
            ...
            mask = gpu2np(self.mask.sum(dim=1)>0)
            fea_fea_emb = fea_fea_emb.detach().cpu().numpy()[mask]
            m = np.zeros(shape=(
    ...
    def updata_m_new(self, fea_fea_emb, init=False):
        ...
        #         self.data_train.data.shape[1], self.hparams.num_pat)) - 0.1
            ...
            #         mode='markers',
            #         marker=dict(
            #             size=[10]*fea_fea_emb.shape[0],
            #             color=kmeans.labels_+1,)))
            # wandb.log({'test':fig})
    ...
    def predict_(self, x):
        ...
        x[:,~self.mask] = 0
        sim = gpu2np(self.forward_simi(x)).reshape(-1,1)
    ...
    def predict_lime(self, x):
        ...
        lat = self(x_b)[2]
        return gpu2np(self.forward_classi(lat))
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.current_epoch == 0 and batch_idx == 0:
            self.data_pt = data1[:num_pt]
            self.label_pt = self.data_train.label[index][:num_pt]
        data = torch.cat([data1, data2, self.data_pt])

        label1 = self.data_train.label[index]
        label2 = self.data_train.label[index]
        # label2 = self.augmentation(index, label1)
        label = torch.cat([label1, label2, self.label_pt])


        # forward
        b_split = (data.shape[0]-num_pt)//2
        pat, mid, lat = self(data)

        mid_1 = mid.reshape(mid.shape[0], -1)[:b_split]
        mid_2 = mid.reshape(mid.shape[0], -1)[b_split:2*b_split]
        mid_tp = mid.reshape(mid.shape[0], -1)[2*b_split:]
        lat_1 = lat.reshape(lat.shape[0], -1)[:b_split]
        lat_2 = lat.reshape(lat.shape[0], -1)[b_split:2*b_split]
        lat_tp = lat.reshape(lat.shape[0], -1)[2*b_split:]
        # rec = self.decoder(lat)
        # loss_rec = torch.norm(rec-data)

        

        # tp = self.tpnn.weight.t().detach()
        # tp += (mid.mean(dim=0)-tp.mean(dim=0))
        # tp *= torch.sqrt(mid.var() / tp.var())
        # tp = tp.detach()
        # tp = mid[torch.randperm(mid.shape[0])[:100]].detach()
        # num_tp = 20
        # randint = torch.randperm(b_split)[:num_tp]
        # randint = torch.arange(start=0, end=1000).to('cuda')
        # _, input_tp, latent_tp = self(data)
        
        # mid.reshape(mid.shape[0], -1)[randint] #+ torch.rand(
        #     # size=(num_tp, mid.shape[1])).to('cuda')/10
        #  = self.forward_modelb(input_tp)
    ...
    def up_mainfig(self, data, mid, mid_old, ins_emb, label, index, mask):
        ...
        data_sampled = self.data_train.data[self.rand_index.to(self.data_train.data.device)]
        # ins_emb_label = gpu2np(self.data_train.label[self.rand_index.to(self.data_train.data.device)])
        ins_emb_data = self(data_sampled)[2].detach().cpu()
        # fea_emb, fea_emb_importance = self.FindFeaEmb(
        #     ins_emb=ins_emb_data,
        #     pat_val=data_sampled.detach().cpu(),
        # )
        # pat_emb = []
        ...
        # self.local_lime(data, label, ins_emb)
        ...
        #     model=self,
        #     cf_index_from=[1, 2],
        #     cf_index_to=self.data_train.data.shape[0]-1,
        #     num_data=20000,
        # )
        # fig.add_trace(go.Scatter(
        #     mode="markers",
        ...
        #     marker_line_width=0,
        #     marker=dict(
        #         size=[5] * ins_emb_data.shape[0],
        #     )
        #     ), row=1, col=3,)
        
        # img_from_emb = gpu2np(self(
        #     torch.tensor(img_from).to(self.data_train.data.device)
        #     )[2])
        # cf_emb = gpu2np(self(
        #     torch.tensor(cf).to(self.data_train.data.device)
        #     )[2])
        # fig.add_annotation(
        ...
        # fig.add_trace(
        #     ec.ShowSankey_Zelin_return_fig(mask, gpu2np(self.PM_root)),
        #     row=2, col=2,
        # )
...
if __name__ == "__main__":  
    ...
    parser.add_argument("--l2alpha", type=float, default=500)
