...
class LitPatNN(LightningModule):
    ...
    def updata_m(self, init=False):
        ...
        if init:
            ...
            return torch.tensor(m).float().to("cpu")
...
def main(args):
    ...
    model = LitPatNN.load_from_checkpoint(
        'save_checkpoint_use/Mnistepoch=1499.ckpt').to('cpu')
    model.m = torch.tensor(np.load(
        'save_checkpoint_use/Mnist_1499.npy')).to('cpu')
    # model = LitPatNN.load_from_checkpoint(
    #     'save_checkpoint_use/Mnistepoch=899.ckpt').to('cpu')
    # model.m = torch.tensor(np.load(
    #     'save_checkpoint_use/Mnist_899.npy')).to('cpu')
    model.eval()
    ...
    pattern_colored_pix = np.zeros(28*28)-1
    ...
    log_dict['c/pattern_colored_pix_imshow'] = px.imshow(
        pattern_colored_pix.reshape(28, 28))
    
    wandb.log(log_dict)

    # cf_index_from = [1, 2, 4, 5, 6, 7, 8, 12, 16, 19]
    cf_index_from = [1, 2, 4, 5, 6]
    ...
    for cf_num in cf_index_to:
        img_to, img_from, cf = cf_expalain.CF_case_study_abili_Proto(
                    ...
                    kappa=args.kappa,
                    theta=args.theta,
                    cf_max_iterations=args.cf_max_iterations,
                    k=args.k,
                )
        ...
        for j in range(cf.shape[0]):
            ...
            fig_all_img.add_trace(go.Heatmap(z=cf_from.reshape(28, 28)[::-1]), 1, j+1)
            fig_all_img.add_trace(go.Heatmap(z=cf_to.reshape(28, 28)[::-1]), 2, j+1)
            fig_all_img.add_trace(go.Heatmap(z=np.abs(cf_from-cf_0).reshape(28, 28)[::-1]), 3, j+1)
            fig_all_img.add_trace(go.Heatmap(z=cf_0.reshape(28, 28)[::-1]), 4, j+1)
    ...
    wandb.log({
        'num_succ_pic':num_succ_pic,
        'num_pix':num_pix/num_succ_pic,
        })
...
if __name__ == "__main__":
    ...
    parser.add_argument("--k", type=int, default=5)  
    parser.add_argument("--cf_max_iterations", type=int, default=1000) 
    parser.add_argument("--cf_num_data", type=int, default=10000) 
