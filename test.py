...
def ShowSankey_Zelin_return_fig(fea_label_pesodu, mask, value):

    mask_np = np.copy(mask)
    mask_np = mask_np * (np.sum(mask_np, axis=1, keepdims=True)>1)
    index_x, index_y = np.where(mask_np>0.5)
    index_x_nodup = np.unique(index_x)
    index_y_nodup = np.unique(index_y)
    index_x_no_dup_sort = np.argsort(index_x_nodup).tolist()
    ...
    value_show = value[mask]

    source = [i for i in range(fea_label_pesodu.shape[0])
            ]+(index_x+ fea_label_pesodu.shape[0]).tolist()
    target = [fea_label_pesodu[i]+fea_label_pesodu.shape[0] for i in range(fea_label_pesodu.shape[0])
            ]+(index_y + index_x.shape[0] +fea_label_pesodu.shape[0]).tolist()
    ...
    Sankey = go.Sankey(
            node = dict(
                ...
                thickness=20, 
                line=dict(color="black", width=0.5), 
                label=['f_{}'.format(i) for i in range(fea_label_pesodu.shape[0])
                     ]+['fg'+str(i) for i in mask.shape[0]
                     ]+['p'+str(i) for i in mask.shape[1]], 
                color=[
                    'green'for i in range(fea_label_pesodu.shape[0])
                    ]+["blue"]*mask.shape[0]+[
                        "red"]*mask.shape[1]
                ),
            link = dict(
...
def Show_global_importance_Zelin_return_fig(p_m):
    ...
    bar = go.Bar(
        x=list_x, y=list_y, orientation='h',
        marker=dict(
            color=['rgb(10,{},10)'.format(int(
            (a-list_x.min())/(list_x.max()-list_x.min()) * 255)
            ) for a in list_x],
        )
    )
...
def show_local_expl(
    ...
    # cluster_dis = sklearn.metrics.pairwise_distances(cluster_centers)
    # cluster_dis[np.eye(cluster_dis.shape[0])>0] = cluster_dis.max()
    # cluster_dis_min = cluster_dis.min(axis=0)
    # cluster_sam_dis = []
    # for c in range(n_clusters):
    #     cluster_sam_dis.append(
    #     sklearn.metrics.pairwise_distances(
    #         cluster_centers[c:c+1],
    #         ins_emb[label_pesodu==c]).mean()
    #     )
    # cluster_sam_dis = np.array(cluster_sam_dis)
    ...
    fea_cluster_centers, r_array = rotate_fea(
        fea=fea_cluster_centers-fea_cluster_centers.mean(axis=0),
        ins=cluster_centers-cluster_centers.mean(axis=0),
        mask = mask_fea_pat,
        )
    fea_emb_all = np.dot(fea_emb_all, r_array)

    fea_emb_all = fea_emb_all+(ins_emb.mean(axis=0)-fea_emb_all.mean(axis=0))
    fea_cluster_centers = fea_cluster_centers+(ins_emb.mean(axis=0)-fea_cluster_centers.mean(axis=0))
    
    fea_emb = []
    fea_emb_name = []
    for f in range(mask_fea_pat.shape[0]):
        # p_per_fea = shap_values_fea_pat[f]
        m_per_fea = mask_fea_pat[f]
        if m_per_fea.sum()>1:
            fea_emb.append(fea_emb_all[f])
            fea_emb_name.append('f_{}'.format(f))
        else:
            fea_emb.append(np.array([None, None]))
            fea_emb_name.append('f_{}'.format(f))
    fea_emb = np.array(fea_emb)

    fea_x = fea_emb[:, 0]
    fea_y = fea_emb[:, 1]
    fea_z = np.zeros_like(fea_x)+1

    # fea_emb_map_to_circle = np.concatenate(
    #     [fea_x[:, None], fea_y[:, None], fea_z[:, None],], axis=1)

    # fea_pat line
    line_x_list = []
    line_y_list = []
    line_z_list = []
    for i in range(mask_fea_pat.shape[0]):
        for j in range(mask_fea_pat.shape[1]):
            if mask_fea_pat[i,j]:
                x_c = [cluster_centers[j][0], fea_cluster_centers[i][0], None]
                y_c = [cluster_centers[j][1], fea_cluster_centers[i][1], None]
                z_c = [0, 1, None]
                line_x_list += x_c
                line_y_list += y_c
                line_z_list += z_c
    ...
    # fea_fea_group line
    line_x_list_fea_group = []
    line_y_list_fea_group = []
    line_z_list_fea_group = []
    for i in range(fea_label_pesodu.shape[0]):
        # for j in range(fea_label_pesodu.shape[1]):
            # if mask_fea_pat[i,j]:
        c = fea_label_pesodu[i]
        x_c = [fea_emb_all[i][0], fea_cluster_centers[c][0], None]
        y_c = [fea_emb_all[i][1], fea_cluster_centers[c][1], None]
        z_c = [1.0, 1, None]
        line_x_list_fea_group += x_c
        line_y_list_fea_group += y_c
        line_z_list_fea_group += z_c
    
    fig_list.append(
        go.Scatter3d(
            x=line_x_list_fea_group,
            y=line_y_list_fea_group,
            z=line_z_list_fea_group,
            mode='lines', 
            opacity=.2,
            textfont=dict(color="grey",),
        )
    ...
    # ins emb
    fig_list.append(
        go.Scatter3d(x=ins_emb[:, 0], y=ins_emb[:, 1], z=np.zeros(shape=(ins_emb.shape[0])),
            mode='markers',
            name='instance emb',
            text=label_pesodu,
            marker_line_width=0,
    ...
    fig_list.append(
        go.Scatter3d(
            ...
            mode='markers',
            name='pat emb',
            text=[str(y) for y in range(cluster_centers.shape[0])],
            marker_line_width=0,
            marker=dict(
                symbol=['square-open']*ins_emb.shape[0],
    ...
    fig_list.append(
        go.Scatter3d(
            x=fea_emb_all[:,0], 
            y=fea_emb_all[:,1], 
            z=[1]*fea_emb_all.shape[0],
            mode='markers',
            name='pat emb',
            # text=fea_emb_name,
            # text=['zzz']*fea_emb_map_to_circle.shape[0],
            marker_line_width=0,
            marker=dict(
                size=[5]*fea_emb_all.shape[0],
                color=fea_label_pesodu,
            ),
            # textfont=dict(
            #     family="sans serif",
            #     size=18,
    ...
    fig_list.append(
        go.Scatter3d(
            x=fea_cluster_centers[:,0], 
            y=fea_cluster_centers[:,1], 
            z=[1]*fea_cluster_centers.shape[0],
            mode='markers',
            name='pat emb',
            # text=fea_cluster_centers,
            # text=['zzz']*fea_emb_map_to_circle.shape[0],
            marker_line_width=0,
            marker=dict(
                size=[15]*fea_cluster_centers.shape[0],
                symbol=['square']*fea_x.shape[0],
                # color=fea_label_pesodu,
            ),
            # textfont=dict(
            #     family="sans serif",
    ...
    if pix > 0:
        ...
        wandb.log(dict_fig)
    return fig_list, mask_fea_pat, shap_values_fea_group, fea_label_pesodu
...
def load_cf_explain(model, ins_emb, cf, img_from):
    ...
    figff = ff.create_quiver(
        x=img_from_emb[usefull_cf, 0], 
        y=img_from_emb[usefull_cf, 1],
        u=cf_emb[usefull_cf, 0]-img_from_emb[usefull_cf, 0], 
        v=cf_emb[usefull_cf, 1]-img_from_emb[usefull_cf, 1],
        scale=1,
        )
    fig_1.append(figff.data[0])

    return fig_1, fig_2klearn.ensemble.RandomForestRegressor(n_estimators=1000)
